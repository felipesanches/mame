...
// USART (one set of definitions; the four USARTs of the ATmega640/1280/2560
// family have identical register layouts at different base addresses)
...
void avr8_device<NumTimers>::device_reset()
{
	...
	for (int n = 0; n < USART_COUNT; n++)
	{
		...
		// UDREn comes out of reset set: the transmit buffer starts empty.
		// UCSRnC resets to 0x06, i.e. eight data bits, no parity, one stop bit;
		// the Arduino core (and so Repetier) relies on that and never writes the
		// register, so getting it wrong silently produces five-bit characters.
...
// Return addresses are pushed low byte first.  Parts with more than 64K words
// of program memory (only the ATmega2560/2561 among the variants modelled here)
// have a 17-bit PC and push and pop a third byte.
...
// Mark an interrupt source as requesting service, or withdraw the request.  The
// request is only recorded here; it is honoured at the next instruction
// boundary by take_interrupt(), which is what makes a request raised while
// SREG.I is clear survive until software re-enables interrupts.
...
// Enter the highest-priority pending interrupt handler.  On the AVR, priority is
// vector order: the lower the vector number, the higher the priority.
void avr8_base_device::take_interrupt()
{
	...
	// Entering the handler is what clears most flags in hardware, so they are
	// cleared here rather than when the condition was first noticed.
	...
	// Four cycles to push the return address and load the vector, plus one more
	// on the parts that push a third byte of PC.
...
// Re-evaluate one interrupt source: it requests service when both its enable bit
// and its flag bit are set.  Which register pair those live in is per-variant
// data, supplied by int_conditions().
...
// ATmega640/1280/1281/2560/2561.  The ATmega644 table above was previously used
// for these parts, which put every interrupt on the wrong vector: TIMER1_COMPA,
// for instance, landed on the ATmega2560's TIMER2_COMPA slot.
...
// One SPI bit period.  Data goes out MSB first on MOSI; the byte that was
// shifted in replaces SPDR when the eighth bit has gone, at which point SPIF is
// raised.  Firmware polls SPIF (or takes the SPI_STC interrupt) to know a
// transfer finished, so leaving it clear stalls any driver that talks SPI.
template <int NumTimers>
...
void avr8_device<NumTimers>::update_timer_waveform_gen_mode(uint8_t t, uint8_t mode)
{
	...
	// NOT static: both tables depend on oc_val and ic_val, which change with the
	// compare registers.  Marked static they would be initialised once, on the
	// first call, and keep the values the registers held at reset forever.
...
// Cycles per bit.  fosc / (16 * (UBRR+1)) normally, fosc / (8 * (UBRR+1)) with
// U2X set, which is what the Arduino core uses to get 115200 out of 16 MHz with
// a 2.1% error rather than 3.5%.
template <int NumTimers>
...
// Character size, from UCSZn2:0.  The 9-bit setting is accepted but the ninth
// bit is not carried through TXB8/RXB8; nothing modelled here uses it.
template <int NumTimers>
...
// Move the byte sitting in the transmit buffer into the shift register and
// build the frame around it.  UDRE goes back up as soon as the buffer is free,
// which is what unblocks a driver that is streaming bytes out.
template <int NumTimers>
void avr8_device<NumTimers>::usart_load_shifter(int n)
{
	...
	// buffer free again, and the transmission is no longer complete
	m_r[base + 0] |= UCSRA_UDRE_MASK;
	m_r[base + 0] &= ~UCSRA_TXC_MASK;
	usart_update_int(n);
}

// One CPU cycle of serial timing, for every USART with something in flight.
template <int NumTimers>
	...
	for (int n = 0; n < USART_COUNT; n++)
	{
		...
		if (m_usart_rx_bits[n] >= 0)
		{
			...
			if (m_usart_rx_count[n] >= int32_t(period))
			{
				...
				{
					// stop bit; a low here is a framing error
					if (!level)
...
// A falling edge on RXDn while the receiver is idle is a start bit.  The first
// data bit is sampled one and a half bit times later, i.e. in the middle of it.
template <int NumTimers>
void avr8_device<NumTimers>::usart_rxd_w(int n, int state)
{
	...
	// Sample in the middle of each bit.  The counter starts half a bit period
	// behind so that the first sample falls one and a half bit times after the
	// falling edge of the start bit, i.e. in the middle of data bit 0 rather
	// than in the middle of the start bit.
	m_usart_rx_shift[n] = 0;
	m_usart_rx_bits[n] = 0;
...
{
	const uint16_t base = usart_base(N);
	// TXC is the only writable flag, and writing a one to it clears it
...
{
	// Reading SPSR with SPIF set and then accessing SPDR is the documented way
	// of clearing SPIF in software.
