...
public:
	...
	///
	/// Slots are identified to the user by their tag, which is
	/// constrained by validity_checker::validate_tag() to lowercase
	/// letters, digits and a handful of punctuation characters, and is
	/// also the command line option name and the key used in the
	/// configuration file.  A slot whose real name in the machine's own
	/// documentation cannot be spelt that way may supply it here for
	/// display purposes; the tag is unaffected.
