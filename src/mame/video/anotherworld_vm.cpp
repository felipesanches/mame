...
void another_world_vm_state::video_start()
{
	...
	//save_item(NAME(m_curPagePtr1));
	//save_item(NAME(m_curPagePtr2));
	//save_item(NAME(m_curPagePtr3));
	save_item(NAME(m_page_bitmaps[0]));
	save_item(NAME(m_page_bitmaps[1]));
	save_item(NAME(m_page_bitmaps[2]));
	save_item(NAME(m_page_bitmaps[3]));
	...
	while (h--)
	{
		...
		while (w--)
		{
			...
			for(int j = 0; j < 4; ++j)
			{
				...
				dest->pix(199-h, x++) = (value >> 4) & 0x0F;
				dest->pix(199-h, x++) = value & 0x0F;
			}
			++src;
		}
	}
...
void VMPolygon::readVertices(const uint8_t *p, uint16_t zoom, const uint8_t *ref, bool video_2, uint8_t level)
{
	...
	*/
	...
	for (int i = 0; i < numPoints; ++i)
	{
		...
		/* Debugging:
		printf("0x%04lX \tx:%d y:%d\n", (unsigned long int) (p - ref), pt->x, pt->y);
		*/
	}
	/* Debugging:
...
void another_world_vm_state::readAndDrawPolygon(uint8_t color, uint16_t zoom, const VMPoint &pt)
{
	...
	if (m_use_video2) {
		...
		printf("CINE-(level=?): 0x%04X \t: readAndDrawPolygon\n", m_data_offset);
	}
	*/
	...
	if (value >= 0xC0)
	{
		...
		m_polygon.readVertices(&m_polygonData[m_data_offset], zoom, m_polygonData, m_use_video2/*, level_bank*/, 0);
...
void another_world_vm_state::updateDisplay(uint8_t pageId)
{
	...
	for (int x=0; x < m_screen->width(); x++)
	{
		...
		{
			uint16_t color = m_curPagePtr2->pix(y, x);
			m_screen_bitmap.pix(y, x) = color;
		}
	}
}
...
void another_world_vm_state::drawPoint(uint8_t color, int16_t x, int16_t y)
{
	...
	if (x >= 0 && x <= 319 && y >= 0 && y <= 199)
	{
		for (int i=0; i<(m_screen->width()/320.0); i++)
		{
			...
			{
				m_curPagePtr1->pix(y+j, x+i) = color;
			}
		}
	}
}
...
void another_world_vm_state::init_palette(palette_device &palette)
{
	//FIXME!
	//changePalette(0);
}
	...
	else
	{
		...
		if (vscroll >= -199 && vscroll <= 199)
		{
			...
			for (int x=0; x < m_screen->width(); x++)
			{
				for (int y=0; y < h; y++)
				{
					dest->pix(dest_y0 + y, x) = src->pix(src_y0 + y, x);
				}
			}
		}
	}
