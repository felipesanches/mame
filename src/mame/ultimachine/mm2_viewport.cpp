...
bool mm2_viewport_device::load_mesh()
{
	...
	if (rd32(base + 4) != 3)
	{
		logerror("%s: mesh asset version %u, expected 3\n", tag(), rd32(base + 4));
		return false;
	}

	const uint32_t body_count = rd32(base + 8);
	const uint32_t mesh_count = rd32(base + 12);
	const uint32_t inst_count = rd32(base + 16);
	m_tri_count = rd32(base + 20);

	static constexpr size_t BODY_BYTES = 12 * 4 + 12 * 4 + 2 * (4 + 3 * 4 + 12 * 4)
			+ 3 * 4 + 4 + 4;
	static constexpr size_t MESH_BYTES = 3 * 4 + 3 * 4 + 4 + 4;
	static constexpr size_t INST_BYTES = 4 + 12 * 4;
	static constexpr size_t TRI_BYTES = 4 + 9 * 2;
	static constexpr size_t HEAD_BYTES = 24;

	if (length < HEAD_BYTES + mesh_count * MESH_BYTES + body_count * BODY_BYTES
			+ inst_count * INST_BYTES + size_t(m_tri_count) * TRI_BYTES)
	{
		logerror("%s: mesh asset is short: %u meshes, %u bodies, %u instances and %u triangles need more than %u bytes\n",
				tag(), mesh_count, body_count, inst_count, m_tri_count, uint32_t(length));
		return false;
	}

	const uint8_t *p = base + HEAD_BYTES;
	...
	m_instances.clear();
	m_instances.reserve(inst_count);
	for (uint32_t i = 0; i < inst_count; i++)
	{
		instance in;
		in.mesh = rd32(p);                                        p += 4;
		for (int j = 0; j < 12; j++)
			in.xform[j] = rdf(p + j * 4);
		p += 48;
	...
	logerror("%s: %u bodies, %u meshes, %u placements, %u triangles\n",
			tag(), body_count, mesh_count, inst_count, m_tri_count);
	return true;
}
...
{
	float body_m[12];
	body_matrix(b, body_m);

	for (uint32_t i = 0; i < b.count; i++)
	{
		const instance &in = m_instances[b.first + i];
		const mesh &me = m_meshes[in.mesh];

		float m[12], t[12], dq[12];
		mat_mul(body_m, in.xform, t);
		mat_identity(dq);
		...
		const uint8_t *tri = m_tris + size_t(me.first) * 22;
		for (uint32_t j = 0; j < me.count; j++, tri += 22)
		{
