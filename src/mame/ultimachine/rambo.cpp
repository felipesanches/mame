...
private:
	...
	output_finder<> m_beads_out;   // how much of the print is being kept
...
TIMER_CALLBACK_MEMBER(rambo_state::thermal_tick)
{
	...
	// IPT_MOUSE_X/Y are relative axes that wrap, so the movement since the last
	// look is the difference taken in the port's own width.  Orbiting only
	// happens while the left button is down.
	...
	// camera keys, sampled rather than edge-triggered so that holding one
	// pans smoothly
...
// 2 ms, so a bead is laid every fifth of a millimetre even during a 100 mm/s
// travel move.
TIMER_CALLBACK_MEMBER(rambo_state::view_tick)
{
	...
	// the part view has to be fed in the same order: set_extrusion() decides a
	// bead was laid by comparing against the position set just before it
	...
	m_viewport->set_full_threads(BIT(viewcfg, 0));
	m_viewport->set_show_3d(!BIT(viewcfg, 1));
...
void rambo_state::rambo(machine_config &config)
{
	...
	*/
	...
	// a second one, drawing only what has been printed; it keeps its own camera
	// and its own copy of the beads
...
// A Metamaquina 2 has no controls at all -- it is driven entirely over its
// serial port.  Everything here moves the camera of the 3D view.
static INPUT_PORTS_START( metamaq2 )
	...
	// The six M8 threaded rods are 77% of every triangle in the machine, because
	// the CAD draws their thread for real.  Both versions are in the mesh asset;
	// the simplified rod paints the thread into its colours instead.
	PORT_START("VIEWCFG")
	PORT_CONFNAME( 0x01, 0x00, "3D view: threaded rod detail" )
	PORT_CONFSETTING(    0x00, "Simplified" )
	PORT_CONFSETTING(    0x01, "As drawn in the CAD (slow)" )
	PORT_CONFNAME( 0x02, 0x00, "3D view" )
	PORT_CONFSETTING(    0x00, "On" )
	PORT_CONFSETTING(    0x02, "Off (faster)" )
INPUT_PORTS_END
...
ROM_START( metamaq2 )
	...
	*/
	...
	/*
	    The machine's own geometry, for the 3D view.  Not a dump: artwork
	    generated from the Metamaquina 2's published OpenSCAD design, in the same
	    way the drivers that use SCREEN_TYPE_SVG ship a vector asset in their ROM
	    set.  The driver runs without it; the 3D view is simply empty.
	*/
	ROM_REGION( 48724424, "meshes", 0 )
	ROM_LOAD( "mm2_meshes.bin", 0, 48724424, CRC(d6c4c093) SHA1(69b1d6d519b4340f23ea47b0e31c9ecface680a4) )
